...
/*   Updated: 2020/02/07 11:19:07 by ehalmkro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_scene draw)
{
	...
	if (delta.x > delta.y)
		pct = get_percent((int)start_end->dt->x, (int)start_end->nxt->dt->x, (int)current.x);
	else
		pct = get_percent((int)start_end->dt->y, (int)start_end->nxt->dt->y, (int)current.y);
